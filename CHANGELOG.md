...
## 22/9/26 - Merge path bridge head: FIX/audit into CHORE/integration-audit-fix-develop-18-9-26-final

### Merge path

- Checked `CHORE/integration-audit-fix-develop-18-9-26-final` against a freshly fetched `origin/develop`: both at `38a516f`, in sync.
- Merge `origin/SCRAP/request-queue-2-clean-integrated`: `git merge origin/SCRAP/request-queue-2-clean-integrated`. Merged clean with no conflicts across 34 files. Committed as `3573a0b`.
- Merge `origin/FIX/audit`: `git merge origin/FIX/audit`. Conflicted in 2 files.

### Conflict resolution

- `nori-primitives/src/storage_layout.rs`: one conflict block, the doc comment on `struct_word_slot`. Took `origin/FIX/audit`'s side, which explains why the word-offset arithmetic holds for the `Request` struct's actual field order (`target`, `slotKey`, `collectionKeysCount`, `collectionKeys`) and what reordering `collectionKeysCount` ahead of `slotKey` would silently break.
- `nori-program/src/mpt.rs`: five conflict blocks, all empty or a subset on `CHORE`'s side. Took `origin/FIX/audit`'s side throughout: the alloy-trie AUDIT REMEDIATION comment documenting the three alloy-trie/alloy-rlp defects, two short AUDIT REMEDIATION notes at `verify_storage_word` and `verify_account`, and two test strengthenings pinning the exact trie-node shapes (`proof_node_kinds`) the exclusion and absence tests walk through.

### Test results

- `cargo test -- --nocapture`: one failure, `consensus_mpt_program_accepts_non_checkpoint_slot` in `nori/tests/1eb72_non_checkpoint_regression.rs`, panicking with `missing field 'queue_storage'` while deserializing its fixture. The fixture predates the queue rewrite's `queue_storage` field on the proof input shape, so it is stale rather than a regression from this merge. Everything else passed.
