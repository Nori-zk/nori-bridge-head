//! Proves `ProofOutputs::to_bytes`/`from_bytes` are inverses of each other
//! for each case, using the actual encoded bytes rather than recomputing
//! them. Consumers that decode or re-assemble these bytes must reproduce
//! every `bytes` value here from the corresponding fields.
//!
//! Also renders the checked cases to
//! `test-vectors/proof-request-queue/proof-outputs-vectors.json`.
//!
//! Run: `cargo test -p nori-sp1-helios-primitives --test proof_outputs_vectors`
...
mod proof_outputs {
    ...
    fn fixture_path() -> PathBuf {
        PathBuf::from(env!("CARGO_MANIFEST_DIR"))
            .join("../test-vectors/proof-request-queue/proof-outputs-vectors.json")
    }
    ...
    fn proof_outputs_round_trip() {
        ...
        .expect("write proof-outputs-vectors.json");
    }
}
