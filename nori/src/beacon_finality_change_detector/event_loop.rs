...
use super::api::BeaconFinalityChangeEventListener;
use anyhow::Result;
use helios_consensus_core::{consensus_spec::MainnetConsensusSpec, types::FinalityUpdate};
use helios_ethereum::{consensus::Inner, rpc::{http_rpc::HttpRpc, ConsensusRpc}};
use log::{info, error};
...
pub struct FinalityChangeDetectorEventLoop {
    ...
    event_listener: Box<dyn BeaconFinalityChangeEventListener + Send + Sync>,
            ...
            match self.check_finality_next_head().await {
                Ok(next_head) => {
                    if next_head > self.current_slot {
                        ...
                        self.event_listener.as_mut().on_beacon_change(next_head).await;
                    }
                }
                Err(e) => {
