...
pub async fn get_source_contract_listener() -> Result<mpsc::Receiver<Result<TokensLockedFilter, anyhow::Error>>> {
    dotenv::dotenv().ok();
    
    // Validate everything upfront
    let eth_ws_rpc = env::var("NORI_ETH_WS_RPC")
        .context("Missing NORI_ETH_WS_RPC in environment")?;
    ...
    tokio::spawn(async move {
        ...
        while let Some(event) = stream.next().await {
            let result = tx.send(
                event.map_err(|e| anyhow::anyhow!("Event error: {}", e))
            ).await;
