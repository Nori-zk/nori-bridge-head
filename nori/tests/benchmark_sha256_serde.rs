...
pub async fn benchmark_finality_update(
    ...
    let encoded_proof_inputs = prepare_zk_program_input(input_head, store_hash, finality_update).await?;
...
async fn benchmark_sha256_serde() {
    ...
    let public_values = benchmark_finality_update(current_slot, finality_update, store_hash).await.unwrap();
